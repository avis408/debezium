...
|
|(Optional)  If using Redis to store offsets, define the hash key in redis. If the `redis.address` configuration is not supplied, and the default value is 'metadata:debezium:offsets'
